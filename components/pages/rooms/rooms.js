...
function Oneroom(props){
    ...
    const [userlist, setUL] = useState([{name:"사용자", Icon:require("../../../image/userbaseIcon.png")}]);
    ...
    return (
        <ScrollView style={styles.subDiv}>
            <View>
                <Image style={styles.headerIcon, {width:'100%', zIndex:2}} source={require("../../../image/selectHeader.png")}/>
            </View>
            ...
            <View style={{padding: 20, borderBottomWidth:4, borderColor:'#f5f5f5'}}>
                <Text style={{fontSize:15,marginBottom:10,fontWeight:'900'}}>습관 설명</Text>
                <Text>{des}</Text>
            </View>
            <View style={{padding: 20, borderBottomWidth:4, borderColor:'#f5f5f5'}}>
                <Text style={{fontSize:15,marginBottom:10,fontWeight:'900'}}>참가자</Text>
            ...
            <View>
            <TouchableOpacity onPress={() => props.viewRoom(null)} style={styles.purpleBtn2}>
...
function Room(props){
    return (
        <TouchableOpacity onPress={() => props.viewRoom(props.id)} style={styles.room}>
...
export default function Rooms(props){
    ...
    return (
            <>
        <ScrollView style={[styles.background, view == null ? {height:670} : {height:1000}]}>
        <StatusBar
                animated={true}
                backgroundColor="#7D63EB"
                barStyle='light-content' />
            <View>
                <Image style={{width:'100%',height : '14%', zIndex:2,}} source={require("../../../image/selectHeader.png")}/>
            ...
            <View>
                <View style={styles.category}>
                    ...
                    /> */}
                    <TextInput style={styles.addHabitTxt} placeholder={""} onChangeText={text => setSearch(text)}  />
                    <TouchableOpacity onPress={() => searchs()} style={[styles.addHabit,{backgroundColor: '#7D63EB', color: 'white'}]}>
                        <Text style={{backgroundColor: '#7D63EB', color: 'white'}}>검색</Text>
                    </TouchableOpacity>
                </View>
                <View>
                <Text style={{fontWeight:'900', fontSize: 18, paddingLeft: 18, color: 'black', paddingTop: 20}}>새로 열린 습관모임</Text>
                <ScrollView>
                    <Room viewRoom={viewRoom} id={1} title={"매일 아침에 일기 쓰기"} include={32} d={1} />
                    <Room viewRoom={viewRoom} id={2} title={"하루 5분 명상하기"} include={32} d={1} />
                </ScrollView>
                </View>
                <Text style={{fontWeight:'900', fontSize: 18, paddingLeft: 18, color: 'black', paddingTop: 20}}>습관모임</Text>
                <ScrollView>
                    <Room viewRoom={viewRoom} id={1} title={"매일 아침에 일기 쓰기"} include={32} d={1} />
                    <Room viewRoom={viewRoom} id={2} title={"하루 5분 명상하기"} include={32} d={1} />
                    <Room viewRoom={viewRoom} id={3} title={"하루 하나 캘리그라피"} include={32} d={1} />
                    <Room viewRoom={viewRoom} id={4} title={"약 먹기"} include={32} d={1} />
                    <Room viewRoom={viewRoom} id={5} title={"운동하기"} include={32} d={1} />
                    
                </ScrollView>
            </View>
            </View>
        </ScrollView>
        {view == null ? undefined : <Oneroom userId={props.email} view={view} viewRoom={viewRoom} />}
        </>
...
const styles = StyleSheet.create({
  ...
  backgroundSub:{position:'absolute', width:'100%',  height:220, zIndex: -1, top: 85},
  onetitle:{
      color: 'white',
      textAlign: 'center',
      fontSize: 20,
